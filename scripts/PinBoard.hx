    ...
    override function create():Void
    {
        ...
        for (i in 0...PIN_RARITIES.length)
        {
            ...
            for (j in 0...PINS_BY_RARITY[i].length)
            {
                ...
                // var isPinUnlocked:Bool = false;
                // var isPinUnlocked:Bool = true;
                if (isPinUnlocked)
                {
                    unlockedPins++;
            ...
            var star:String = unlockedPinsPerRarity[i] == PINS_BY_RARITY[i].length ? '${PTIcon.Star} ' : '';
